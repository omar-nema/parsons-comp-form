...
function setup() {
    ...
    createP('Scale')
    sliderScale = createSlider(.1,2,1,.1);
...
function draw() {
    ...
    for (x=0; x<imgW; x++){
        for (y=0; y<imgH; y++){
            //var index = x*imgW + y
            var index = x+y*imgW;
            //convert to 2d (Vector), and then you can det. proximity to point for localized distortion
            pixel = imgPixels[floor(index + noise(index*.02)*40)]
            colr = color(pixel[0], pixel[1],pixel[2],pixel[3])
            stroke (colr)
            fill(colr);
            img_scale = sliderScale.value();
            //x*scale and then the size is the scale
            rect(x*img_scale,y*img_scale,img_scale,img_scale)
        }
